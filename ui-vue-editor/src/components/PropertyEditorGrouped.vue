        ...
        <t-collapse-panel v-for="group in groupedProperties" :key="group.key" :header="group.label">
          <t-form-item v-for="property in group.items" :key="property.name" :label="property.label"
            ...
            <t-input v-if="property.type === 'string'" v-model="formData[property.name]"
              :placeholder="`请输入${property.label}`" @change="handleStringInput($event, property.name)" />

            <!-- 数字属性 -->
            <t-input-number v-else-if="property.type === 'number'" v-model="formData[property.name]"
              :placeholder="`请输入${property.label}`" @change="handleNumberInput($event, property.name)" />

            <!-- 布尔属性 -->
            <t-switch v-else-if="property.type === 'boolean'" v-model="formData[property.name]"
              @change="handleBooleanChange($event, property.name)" />

            <!-- 选择属性 -->
            <t-select v-else-if="property.type === 'select'" v-model="formData[property.name]"
              :placeholder="`请选择${property.label}`" @change="handleSelectChange($event, property.name)">
              <t-option v-for="option in property.options" :key="option.value" :value="option.value"
                :label="option.label" />
            </t-select>

            <!-- 颜色属性 -->
            <t-color-picker v-else-if="property.type === 'color'" v-model="formData[property.name]"
              @change="handleColorInput($event, property.name)" />

            <!-- 表达式属性 -->
            <t-textarea v-else-if="property.type === 'expression'" v-model="formData[property.name]"
              :placeholder="`请输入${property.label}`" :autosize="{ minRows: 3, maxRows: 6 }"
            ...
            <t-textarea v-else-if="property.type === 'method'" v-model="formData[property.name]" placeholder="输入方法名称"
              :autosize="{ minRows: 3, maxRows: 6 }" @change="handleTextareaInput($event, property.name)" />

            <!-- 默认输入 -->
            <t-input v-else v-model="formData[property.name]" :placeholder="`请输入${property.label}`"
